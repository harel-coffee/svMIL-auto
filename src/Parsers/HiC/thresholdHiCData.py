...
outDir = "../../../data/thresholdedHiC/" + interactionType + "/"

threshold = 5 #For now I only use a uniform threshold, other thresholds could be implemented

pickleExists = True #use this switch to use a pickle file from a previous run if already generated, then the data does not need to be read again
...
for hiCFilePath in filesToRead:
	...
	with open(hiCFile[0], "r") as f:
		...
		outFile = outDir + fileName + "_thresholded_" + str(threshold)
